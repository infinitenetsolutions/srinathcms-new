...
$grade_total = '';
// for total student are present and absent 
if (isset($_GET['grade'])) {
    $grade = $_GET['grade'];
    if ($grade != "all") {
        $grade_qury = "SELECT *
        FROM tbl_attendance WHERE grade_id='$grade' GROUP BY student_id;";
        $student_result = mysqli_query($connection, $grade_qury);
    } else {
        $grade_qury = "SELECT *  FROM tbl_attendance  GROUP BY student_id;";
        $student_result = mysqli_query($connection, $grade_qury);
    }
} else {
    $grade_qury = "SELECT * FROM tbl_attendance GROUP BY student_id ;";
...
// getting the total number of student persent and absent in class
if (isset($_GET['grade'])) {
    $grade = $_GET['grade'];
    if ($grade != "all") {
        // persent total of the attandance by grade
        $grade_qury = "SELECT * FROM `tbl_attendance` WHERE `grade_id`='$grade' && `attendance_status`='Present'";
        $grade_result = mysqli_query($connection, $grade_qury);
        $present_total = mysqli_num_rows($grade_result);

        // Absent total of the attandance by grade
        $grade_qury = "SELECT * FROM `tbl_attendance` WHERE `grade_id`='$grade' && `attendance_status`='Absent'";
        $grade_result = mysqli_query($connection, $grade_qury);
        $absent_total = mysqli_num_rows($grade_result);
    } else {
        // persent total of the attandance

        $grade_qury = "SELECT * FROM `tbl_attendance` WHERE `attendance_status`='Present'";
        $grade_result = mysqli_query($connection, $grade_qury);
        $present_total = mysqli_num_rows($grade_result);

        // Absent total of the attandance
        $grade_qury = "SELECT * FROM `tbl_attendance` WHERE `attendance_status`='Absent'";
        $grade_result = mysqli_query($connection, $grade_qury);
        $absent_total = mysqli_num_rows($grade_result);
    }
} else {
            ...
            <div class="row">
                <div class="col-md-9">
                    ...
                    <div class="col-sm-3 text-success">
                        Present Total :- <b> <?php echo $present_total ?></b>
                    </div>

                    <div class="col-sm-3 text-danger">
                        Absent Total :- <b> <?php echo $absent_total ?></b> </div>

                    <div class="col-sm-3">
                        Total Attendance :- <b> <?php echo $absent_total + $present_total ?></b> </div>
                </div>
...
<script>
    function changeGrade() {
        ...
        window.location.href = "finalreport.php?grade=" + changeValue;
    }
</script>
<script>

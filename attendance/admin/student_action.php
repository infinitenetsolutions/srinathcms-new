...
{
	if($_POST["action"] == "fetch" && !empty($_GET["grade"]) && $_GET["grade"] != "all")
	{
		$query = "
		SELECT * FROM tbl_student 
		INNER JOIN tbl_grade 
		ON tbl_grade.grade_id = tbl_student.student_grade_id 
		WHERE tbl_student.student_grade_id  = ".$_GET["grade"]." AND (
		";
		...
		if(isset($_POST["search"]["value"]))
		{
			...
			OR tbl_student.parent_mob_no_1 LIKE "%'.$_POST["search"]["value"].'%" 
			OR tbl_student.parent_mob_no_2 LIKE "%'.$_POST["search"]["value"].'%" 
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = $row["parent_mob_no_1"];
			$sub_array[] = $row["parent_mob_no_2"];
	...
	} else if($_POST["action"] == "fetch" && $_GET["grade"] == "all"){
	            $query = "
        		SELECT * FROM tbl_student 
        		INNER JOIN tbl_grade 
        		ON tbl_grade.grade_id = tbl_student.student_grade_id 
	...
	if($_POST["action"] == 'Add' || $_POST["action"] == "Edit")
	{
		...
		$error_parent_mob_no_1 = '';
		$error_parent_mob_no_2 = '';
		...
		else
		{
			...
			if(empty($_POST["parent_mob_no_1"]))
		{
			$error_parent_mob_no_1 = 'Parent Number is required';
			$error++;
		}
		else
		{
			...
			if(empty($_POST["parent_mob_no_2"]))
		{
			$error_parent_mob_no_2 = 'Parent Number is required';
			$error++;
		}
		else
		{
		...
		if($error > 0)
		{
			$output = array(
				...
				'error_parent_mob_no_1'		    =>	$error_parent_mob_no_1,
				'error_parent_mob_no_2'	    	=>	$error_parent_mob_no_2,
	...
	if($_POST["action"] == "edit_fetch")
	{
		...
		if($statement->execute())
		{
			...
			foreach($result as $row)
			{
				...
				$output["parent_mob_no_1"] = $row["parent_mob_no_1"];
				$output["parent_mob_no_2"] = $row["parent_mob_no_2"];

...
if ($chk_tx == "success") {
    ...
    // $particular_paid_id = json_decode($_SESSION["particular_paid_id"]);
    // $particular_paid_amount = json_decode($_SESSION["particular_paid_amount"]);
    ...
    $sql = "INSERT INTO `tbl_fee_paid`
                                ( `student_id`, `course_id`, `particular_id`, `paid_amount`, `rebate_amount`, `fine`, `balance`, `payment_mode`, `cash_deposit_to`, `cash_date`, `notes`, `receipt_date`, `bank_name`, `transaction_no`, `transaction_date`, `receipt_no`, `paid_on`, `university_details_id`, `fee_paid_time`, `payment_status`, `status`) 
                                VALUES 
                                ('$registrationNumber', '$courseId', '$implodedId', '$implodedAmount', '$rebate_amount', '$fine_amount', '$remaining_amount', '$PaymentMode', '$cashDepositTo', '$paymentDate', '$NotesByAdmin', '$paidDate', '$bankName', '" . $result["data"]["txnid"] . "', '$paymentDate', 'SU_$receipt_no_gen', '$paymentDate', '$academicYear', '$date_variable_today_month_year_with_timing', 'cleared', '$visible')
                                ";
   ...
   ";
    ...
    for ($i = 0; $i < count($perticulars); $i++) {
       ...
       (`reg_no`,	`course`,`academic_year`, `received_date`, `particulars`, `amount`, `payment_mode`,`check_no`,`bank_name`,`income_from`,`post_at`) 
       VALUES
    ...
    if ($con->query($sql)) {
        ...
        $_SESSION['name'] = $result["data"]["firstname"];
...
?>
    ...
    <head>
        ...
        <title>SRINATH UNIVERSITY | Student Fee Details </title>
        <!-- Fav Icon -->
